...
func (t KdTree) binarySearch(x NodeData, compareLat bool) (int, bool) {
	...
	index, linearSearch := KdTree{t.Nodes[middle+1:], t.Data}.binarySearch(x, !compareLat)
	return middle + 1 + index, linearSearch
}

func (t KdTree) linearSearch(x NodeData) int {
	for i := range t.Nodes {
...
// linearSearch performs a line search where one coordinate is fixed and the other one is minimized
...
/*func (t KdTree) lineearSearch(startIndex int, x NodeData, searchLat bool) int {
