...
func aStarHeuristic(g graph.Graph, curr graph.Node, n graph.Node, distOfEndNodes float64) float64 {
	...
	dist := geo.Distance(geo.Coordinate{Lat: lat1, Lng: lng1}, geo.Coordinate{Lat: lat2, Lng: lng2}) - distOfEndNodes
	if dist < 0 {
		return 0
	}
	return dist
...
func distanceOfNodes(g graph.Graph, t []graph.Way) float64 {
	...
	if len(t) > 1 {
		...
		distOfNodes = geo.Distance(geo.Coordinate{Lat: endLat0, Lng: endLng0}, geo.Coordinate{Lat: endLat1, Lng: endLng1})
	}
	return distOfNodes
}
