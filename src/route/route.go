	...
	for i := 0; i < len(waypoints)-1; i++ {
		...
		if allequal {
			...
			if len(startWays) == 1 { // If the end node is on the edge outgoing from s
				...
				legs[i] = Leg{FormatDistance(legDistance), MockupDuration(legDistance), step1.StartLocation, step2.EndLocation, steps}
			}
		} else {
			...
			if getDistance(g, startWays[0].Node, endWays[0].Node) > 100.0*1000.0 { // > 100km
				...
				legs[i] = PathToLeg(g, dist, vertices, edges, start, end)
			} else {
